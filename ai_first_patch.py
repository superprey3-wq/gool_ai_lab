...
Hard safety gates (warm-up, halftime and post-goal cooldown) remain non-overridable.
"""
...
def _prompt(payload):
    return "Ты главный LIVE-исследователь экспериментальной системы GOOL AI LAB. Твоя задача — решить, оправдан ли вход на ЕЩЁ ОДИН гол после текущей точки. GOOL MASTER и внутренние стратегии — только признаки, а не приказ. Учитывай минуту, счёт, xG, удары, удары в створ, big chances, действия в штрафной, угловые, давление, momentum, историю стратегий и оставшееся время. ENTER = вход оправдан сейчас; WATCH = интересно, но пока рано; REJECT = слабый вход. Не придумывай отсутствующие данные. Верни только JSON по схеме.\n\nDATA:\n"+json.dumps(payload,ensure_ascii=False)
...
def _ask(payload):
    ...
    body={"contents":[{"role":"user","parts":[{"text":_prompt(payload)}]}],"generationConfig":{"temperature":0.1,"maxOutputTokens":320,"response_mime_type":"application/json","response_schema":_SCHEMA}}
    ...
    verdict["latency_s"]=round(time.monotonic()-started,2);return verdict,None
...
def _evaluate(m,s,p,goals,market):
    ...
    # Safety is not delegated to the model. It can control football selection, but
    # never bypass warm-up/halftime/post-goal protection.
    ...
    if verdict:
        decision=str(verdict.get("decision") or "REJECT").upper();prob=int(verdict.get("goal_probability") or 0);ai_enter=decision=="ENTER" and prob>=MIN_PROB;ai_route=f"AI_{decision}_{prob}"
        ...
        _append({"ts":int(time.time()),"payload":payload,"verdict":verdict,"final_enter":ai_enter});return ai_enter,ai_route,master,sc,hz,mkt
