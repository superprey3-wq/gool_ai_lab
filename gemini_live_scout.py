...
MINUTE_MIN=int(os.getenv("GEMINI_SCOUT_MINUTE","10")); MINUTE_MAX=int(os.getenv("GEMINI_SCOUT_MAX_MINUTE","88"))
ENTER_PROB=int(os.getenv("GEMINI_SCOUT_ENTER_PROB","70")); WORKERS=max(2,int(os.getenv("GEMINI_SCOUT_WORKERS","4")))
RECHECK=max(120,int(os.getenv("GEMINI_SCOUT_RECHECK_SECONDS","240")))
MAX_AI_PER_CYCLE=max(1,int(os.getenv("GEMINI_SCOUT_MAX_AI_PER_CYCLE","8")))
_seen={}
...
def useful(s):
 keys=("xg","shots","shots_on_target","big_chances","corners","shots_inside_box","touches_box")
 return sum(sum(pair(s,k)) for k in keys)>0

def prompt(p):
 return """Ты независимый профессиональный футбольный LIVE-аналитик. Твоя специализация — ловить ЕЩЁ ОДИН ГОЛ в текущем матче (рынок тотал больше от текущего счёта). Ты не знаешь решений GOOL, MASTER, MATH или других моделей и не должен имитировать алгоритм. Анализируй ТОЛЬКО предоставленные сырые LIVE-данные Flashscore: минуту, счёт, xG, удары, удары в створ, big chances, действия/касания в штрафной, угловые и уже забитые голы. Оцени характер счёта: нужна ли команде атака, не убит ли матч крупным преимуществом, достаточно ли времени. ENTER ставь только когда сам видишь убедительное основание ждать минимум один следующий гол. WATCH — потенциал есть, но сейчас рано. REJECT — гол не выглядит достаточно вероятным. NO_DATA — статистики недостаточно. Не выдумывай отсутствующие показатели. goal_probability — твоя честная вероятность хотя бы одного следующего гола до конца матча. horizon_minutes — наиболее вероятный горизонт следующего гола, если он будет. reason — кратко 1-2 главных фактора. risk — главный риск прогноза. Верни только JSON по схеме.\nDATA:\n"""+json.dumps(p,ensure_ascii=False)
...
def fetch_one(m):
 try:
  raw=fetch_stats(m.event_id);s=parse_stats(raw) if raw else {}
  return m,s
 except:return m,{}
def goals(m):
 try:
...
def scan(live):
 ...
 if c:
  with ThreadPoolExecutor(max_workers=min(WORKERS,len(c))) as ex:
   fs=[ex.submit(fetch_one,m) for m in c]
   for f in as_completed(fs):m,s=f.result();stats[str(m.event_id)]=s
 # cheap data-only prioritisation: richest attacking stat lines first, not GOOL scoring
 def activity(m):
 ...
 ready=[m for m in c if useful(stats.get(str(m.event_id),{}))];ready.sort(key=activity,reverse=True)
 checked=sent=0
 for m in ready[:MAX_AI_PER_CYCLE]:
  ...
  d=str(v.get("decision") or "");prob=int(v.get("goal_probability") or 0)
  logger.warning("GEMINI_SCOUT %d' %s — %s %d:%d | %s %d%% | model=%s",m.minute,m.home,m.away,m.home_score,m.away_score,d,prob,model)
  if d=="ENTER" and prob>=ENTER_PROB:
   if send(m,v,model):sent+=1
 logger.info("GEMINI_SCOUT_CYCLE live=%d data_candidates=%d ai_checked=%d sent=%d",len(live),len(ready),checked,sent)
 return sent
